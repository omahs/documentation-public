...
Orderly Network uses the format of `<TYPE>_<BASE>_<QUOTE>` to represent a symbol name, for example: `PERP_BTC_USDC` means that it is `BTC_USDC` pair in `PERP` trading market.
