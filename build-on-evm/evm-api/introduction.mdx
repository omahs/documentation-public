...
Please set the corresponding header in your request. See refer to [Authentication](#authentication) for more information.

## Symbol  
Orderly Network uses the format of `<TYPE>_<BASE>_<QUOTE>` to represent a symbol name, for example: `SPOT_NEAR_USDC.e` means that it is `NEAR_USDC.e` pair in `SPOT` trading market.
...
Specific error codes and messages are defined in [Error Codes](#error-codes).