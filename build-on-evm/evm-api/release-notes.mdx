...
---
## 2024-01-09
Added get account details API

Added get user statistics API

Added get user fee tier API

Added create algo order, edit algo order, cancel algo order and get algo order APIs (Only STOP orders are supported for now)

Added address and order_tag parameter in get broker daily statistics API

Added liquidation_tier parameter for each symbol

Added symbols array and high/low risk perps in claim liquidated positions API

Added allow_cross_chain_withdrawal parameter to create withdraw request API

Added PENDING_REBALANCE status for withdrawal request in REST and WSS
