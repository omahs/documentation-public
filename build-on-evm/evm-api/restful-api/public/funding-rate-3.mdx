---
title: "Get funding rate history for one market"
---


<RequestExample>


```shell cURL
curl 'https://api-evm.orderly.org/v1/public/funding_rate_history?symbol=PERP_ETH_USDC.e&page=1&size=5'
```

```python Python
import requests

url = "https://api-evm.orderly.org/v1/public/funding_rate_history?symbol=PERP_ETH_USDC.e&page=1&size=5"

payload={}
headers = {}

response = requests.request("GET", url, headers=headers, data=payload)
```

```javascript JavaScript
var axios = require('axios');

var config = {
   method: 'get',
   url: 'https://api-evm.orderly.org/v1/public/funding_rate_history?symbol=PERP_ETH_USDC.e&page=1&size=5',
   headers: {}
};

axios(config)
.then(function (response) {
   console.log(JSON.stringify(response.data));
})
.catch(function (error) {
   console.log(error);
});
```
</RequestExample>

**Limit: 10 requests per 1 second per IP address**


`GET /v1/public/funding_rate_history`


Get funding rate history for one market.


<ResponseExample>


```json

{
   "data":{
      "meta":{
         "current_page":1,
         "records_per_page":5,
         "total":13
      },
      "rows":[
         {
            "funding_rate":0.0001,
            "funding_rate_timestamp":1684224000000,
            "next_funding_time":1684252800000,
            "symbol":"PERP_ETH_USDC.e"
         },
         {
            "funding_rate":0.0001,
            "funding_rate_timestamp":1684195200000,
            "next_funding_time":1684224000000,
            "symbol":"PERP_ETH_USDC.e"
         },
        ...
      ]
   },
   "success":true,
   "timestamp":1684232331309
}

```

</ResponseExample>
**Parameters**


| Name      | Type      | Required      | Description                                                                                 |
| ----------|-----------|---------------|---------------------------------------------------------------------------------------------|
| symbol    | string    | Y             |                                                                                             |
| start_t   | timestamp | N             |start time range that you wish to query, noted that the time stamp is a 13-digits timestamp. |
| end_t     | timestamp | N             |end time range that you wish to query, noted that the time stamp is a 13-digits timestamp.   |
| page      | number    | N(default: 1) |the page you wish to query.                                                                  |
| size      | number    | N             |Default: 60                                                                                  |


