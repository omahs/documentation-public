...
``` shell cURL
curl 'https://api-evm.orderly.org/v1/public/volume/stats'
```

```python Python
import requests

url = "https://api-evm.orderly.org/v1/public/volume/stats"
...
Get the latest volume statistics of Orderly and its associated brokers. Note that for broker volume, the volume is counted as the sum of the maker and taker volume, while for the full Orderly platform, the volume is the total amount matched on the platform (i.e. taker and maker are not double-counted.)
