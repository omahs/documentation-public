...
Orderly Network uses the ed25519 elliptic curve standard for Orderly keys. Users can generate Orderly keys using any public ed25519 libraries and then be added to any account through a REST API.
...
| expiration | timestamp | Y            | Expiration time of the key in UNIX millisecondsMaximum allowed expiration is 365 days from add |
  ...
  <Step title="Add Orderly key">
     Send all the necessary information via [Add Orderly key API](https://orderly.network/docs/build-on-evm/evm-api/restful-api/public/add-orderly-key).
  </Step>
</Steps>
