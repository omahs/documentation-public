...
<Steps>
  ...
  <Step title="Calculating the fee rates">
    ...
    Orderly has released a tool for assigning users to fee tiers based on volume to save brokers the hastle of building it from scratch. Source code can be found on out [GitHub](https://github.com/OrderlyNetwork/orderly-evm-broker-fee-admin).
    <Note>
    ...
    Set the `default` fee rate through [POST `/v1/broker/fee_rate/default`](/build-on-evm/evm-api/restful-api/private/update-default-broker-fee).
  </Step>
  <Step title="Setting the fees">
    ...
    2. **For users re-assigned to "default" fee rate:** send the list through [POST `/v1/broker/fee_rate/set_default`](/build-on-evm/evm-api/restful-api/private/update-default-user-fee-rate)
    ...
    Retrieve default fee rate through [GET `/v1/broker/fee_rate/default`](/build-on-evm/evm-api/restful-api/private/get-default-broker-fee).
  </Step>
</Steps>
...
Front-ends can develop a custom tiered structure based on different parameters like volume, staked tokens, etc.
  </Accordion>

  <Accordion title="Can I get help determining and setting fees for the users?">
We have built a tool to help brokers set and assign users to volume based TimeRanges.
  </Accordion>
...
Broker fee rebates are distributed daily to broker's Orderly account registered with the wallet provided during the broker_id setup.
  </Accordion>
