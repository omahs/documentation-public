...
$$ Q_{final}=\sum_{N=1}^{totalEpochMinutes} \{Q_{min(N)}=[\dfrac{BidDepth_1}{Spread_1}+\dfrac{BidDepth_2}{Spread_2}...., \dfrac{AskDepth_1}{Spread_1}+\dfrac{AskDepth_2}{Spread_2}...]\}^{0.35} *[\dfrac{\sum_{N=1}^{totalEpochMinutes}Count(Q_{min(N)}>0)}{totalEpochMinutes}]^5 * [max(10,stakedORDER)]^{0.2} * [MakerVolume]^{0.45}$$
