...
7. \$ORDER (ERC-20) will be received by users when claiming unstaked \$ORDER on Ethereum L1. 
...
## Redeem VALOR
- The price of VALOR will be determined by the Treasury pool divided by the total amount of VALOR, with the Treasury pool being the cumulative 70% of Orderly's net fee.
...
- VALOR price serves as a current estimated value of how much USDC is redeemable for each VALOR. The formula is as follows:
    ...
    - VALOR price \= (total USDC in treasury) / (total circulating supply of VALOR)
        ...
        - The total amount of USDC in the treasury is updated daily.
            ...
            - Orderly will upload 70\% of the protocol’s day net revenue amount to the treasury.
...
The value of VALOR can only increase over time as it ties in with the protocol’s revenue from trading fees earned. As long as Orderly earns trading fees, 70\% of the net revenue will be injected daily into the treasury. In short, the later one redeems their VALOR, the more USDC they will receive per VALOR.
...
Daily returns refer to the 70\% of the protocol daily net revenue divided by the daily USD equivalent of total staked \$ORDER and esORDER.
