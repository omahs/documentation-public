...
**Referral Code Creation:** Brokers create unique referral codes linked to specific user accounts.
...
**Taker Fee Rebate:** The referral rebate applies exclusively to taker fees.

### Distribution of Rebates:
- **Referee Rebate:** A portion of the taker fee is rebated to the referee.

- **Referrer Rebate:** A matching portion of the taker fee is rebated to the referrer.

- **Broker Rebate:** The remaining portion of the taker fee is rebated to the broker.
...
-Total Taker Fee for Referee's Order: 7 basis points (bps) = 15 USDC

-Broker Rebate: 4 bps ≈ 8.6 USDC
...
- Broker Rebate: 5.2 USDC

## User Flow for Brokers
<Steps>
  <Step title="Generating referral codes">
    Brokers generate and assign unique referral codes for each user (referrer) through [POST `/v1/referral/create`](/build-on-evm/evm-api/restful-api/private/create-referral-code).

    Taker rebate need to provided for both referee and referrer. 

    <Note>
This endpoint is authenticated with broker's admin account
    </Note>
  </Step>
...
Brokers can retrieve details of referees, referrers, rebates. All endpoints listed under ["Referral Program"](/build-on-evm/evm-api/restful-api/private/get-referral-info) section.
  </Step>
</Steps>
